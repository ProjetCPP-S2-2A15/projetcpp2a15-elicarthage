...
Arduino::~Arduino() {
    ...
    delete serialPort; // Correct
}

void Arduino::initSerial() {
    serialPort = new QSerialPort();
    serialPort->setPortName("COM5");  // ⚠️ Remplace COM5 par le port utilisé par ton Arduino
...
void Arduino::lireNomDepuisArduino() {
    ...
    for (int i = 0; i < lignes.size() - 1; ++i) {
        ...
        // Affichage du nom reçu dans la sortie Qt
        ...
        if (nomPrenom.size() == 2) {
            ...
            if (query.next()) {
                ...
                // Mettre à jour l'état de présence à 'P'
    ...
    buffer = lignes.last();  // Conserve le reste (s'il y en a)
}
}
